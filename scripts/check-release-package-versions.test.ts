...
function fixture(rootVersion = "2.6.0", coreVersion = "0.2.0") {
  return {
...
test("release package checker accepts current root release identities", () => {
  assert.doesNotThrow(() => validateReleasePackageVersions(fixture()));
  assert.doesNotThrow(() => validateReleasePackageVersions(fixture("2.7.0", "0.2.1")));
});

test("simulated targeted Release Please updates change only root and dependent lock identities", () => {
  ...
  assert.deepEqual(changedPaths(before.lockfile, after.lockfile), ['$.packages.[""].dependencies.@kontourai/console-core', "$.packages.console-core.version", "$.packages.console-server.dependencies.@kontourai/console-core", "$.packages.console-ui.dependencies.@kontourai/console-core"]);
});
...
for (const [label, mutate, expected] of [
  ...
  ["missing Console Server root exclusion", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["."]["exclude-paths"] = ["console-core", "console-ui"]; }, /root package must exclude console-server ownership/],
  ...
  ["missing Core release updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"] = []; }, /Core package must declare exactly six/],
  ["wrong Core root dependency updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"][3].jsonpath = "$.dependencies['@kontourai/console-core']"; }, /must target \/package-lock\.json \$\.packages\[''\]\.dependencies/],
  ["wrong Core Console Server manifest updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"][1].path = "/packages/console-server/package.json"; }, /must target \/console-server\/package\.json/],
  ["wrong Core Console Server lock updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"][2].jsonpath = "$.packages['console-server'].dependencies.core"; }, /must target \/package-lock\.json \$\.packages\.console-server\.dependencies/],
  ["wrong Core Console UI lock updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"][4].jsonpath = "$.packages['console-ui'].dependencies.core"; }, /must target \/package-lock\.json \$\.packages\.console-ui\.dependencies/],
  ["wrong Core Console UI manifest updater", (value: ReturnType<typeof fixture>) => { value.releaseConfig.packages["console-core"]["extra-files"][5].path = "/packages/console-ui/package.json"; }, /must target \/console-ui\/package\.json/],
