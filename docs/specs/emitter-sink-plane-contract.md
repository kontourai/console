...
`kontour serve` exposes that local hub over a loopback HTTP server for development. Its v0 boundary is deliberately narrow: bind locally by default, accept `POST /records`, expose `GET /state`, `GET /inspect`, and `GET /events` server-sent events, persist through local files, and do not introduce a database, auth model, remote execution channel, product API fetcher, or action executor. Hosted Console work must add those concerns explicitly instead of treating the local dev server as production security or storage architecture.
