...
`sideEffect` and `confirmation` reuse the exact vocabulary `ProductCapabilityDescriptor` commands already declare (`ProductCommandSideEffect`, `ProductCommandConfirmation` — see [Product Capability Descriptor Protocol](product-capability-descriptor.md#descriptor-fields)), so a host publishing both a CLI descriptor and a UI binding for the same command states its policy once. `intentBindingFromCommand(descriptor, commandPath, execute)` derives a `HostIntentBinding` from a command *actually present* in `descriptor.commands` — it looks the command up by `commandPath` inside the supplied descriptor rather than accepting a free-floating command object, and rejects the result (`{ ok: false, error: "command-not-found" }` or `{ ok: false, error: "authority-mismatch" }`) unless the found command's own `authority.productId` equals `descriptor.product.id`. This closes the laundering path a hand-assembled or unvalidated descriptor could otherwise exploit: a command declaration copied from one product's descriptor can never be bound under a different product's label, even if it is positionally present in that other descriptor's `commands` array. `resolveIntentBinding` itself has no way to re-derive this guarantee — it trusts the `(product, command)` labels on whatever `HostIntentBinding` it is given, so `intentBindingFromCommand` (not hand-authored bindings) is the provenance-checked path, and the CLI router path (`@kontourai/cli`, [Kontour CLI Router](kontour-cli-router.md)) and a UI binding for the same authority can share one declared, verified policy instead of drifting.
...
- an intent with `confirmation: "user-request"` or `"operator-request"` executes **only** if the host supplied a `confirm(intent, resolution) => unknown` gate and that gate resolves to the **literal boolean `true`** — any other resolved value (`"yes"`, `1`, `{}`, `0`, `""`, `null`, `undefined`, `false`, ...) withholds execution exactly like an explicit decline; there is no truthy-coercion shortcut;
...
- if `confirm` throws synchronously or its returned promise rejects, execution is withheld exactly the same way, and `bindIntentHandler` routes the error to an optional `onConsentError` observer instead of letting it propagate uncontrolled or become an unhandled promise rejection;
...
This means the never-authority invariant is enforced by construction at every layer that can reach `execute`: `resolveIntentBinding` never returns an `execute` for an authority the host did not declare, `intentBindingFromCommand` never produces a binding for a command that is not both a real member of the descriptor and authority-matched to it, and `bindIntentHandler` never calls the `execute` it did return unless the resolved confirmation tier's gate actually settled to literal `true`. A host is still responsible for its own confirmation UI (a dialog, an inline second click, an operator-approval flow) — this spec defines the gate's contract, not its presentation.

## Accepted residual risk: TOCTOU is mitigated, not eliminated (2026-07-22)

Any host whose `execute` resolves an on-disk executable PATH and then spawns it — the standalone action runner (console#232/C5) is the reference case in this repo; see `@kontourai/cli`'s `resolveStandaloneProductBinding` in `cli/src/standalone-runner.ts` — has an inherent time-of-check-to-time-of-use (TOCTOU) window between "this file was just verified safe" and "the OS actually opens this file for execution." Consent (§3 above) can be arbitrarily delayed — a user may sit on a confirmation dialog for minutes — so a naive implementation that resolves the executable once, at bind time, and reuses that decision at execute time would let an attacker who can write to the resolved path (or an ancestor directory) between those two points substitute an arbitrary binary for whatever the user actually agreed to run.

The mitigation the reference standalone-runner implementation uses: re-resolve AND re-validate containment (the SAME real-path/symlink-escape checks used at bind time — `resolveLocalProductExecutable`, `@kontourai/console-core`) from scratch, unconditionally, immediately before every spawn, not just the first. This shrinks the exploitable window from "the whole consent-wait duration" (seconds to indefinite) down to "the time between one `fs.realpath`/`fs.stat` call returning and `child_process.spawn`'s underlying `execve` call" (sub-millisecond, typically sub-microsecond in practice).

This is the strongest practical mitigation available in this runtime, and it is NOT full atomicity — this spec does not claim it is. Node's `child_process` module has no `fexecve`-style "execute this already-verified file descriptor" primitive; `spawn` always launches BY PATH, so a swap landing inside that final sub-microsecond window is not detectable by any check a pure-JS process can perform before calling `spawn`. The threat model this residual risk requires: an attacker with WRITE access to the product's own installed binary (or its containing directory) on the SAME machine, timed to land inside that specific instant. That level of access is already a full local compromise of that product's installation — an attacker who can win that race could, with the identical access, simply overwrite the binary permanently (no race required) and wait for the next ordinary invocation, standalone runner or not. The standalone runner therefore grants no capability beyond what that pre-existing compromise already has; it neither introduces a new privilege boundary nor silently claims to close a race that no path-based `exec` primitive in Node can close.
