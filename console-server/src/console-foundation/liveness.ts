 ...
 * `id` when the emitter omits one (relay.sh never sets `id`). The synthesized id
 * is deterministic over (actor, subjectId, type, at) so an exact-duplicate retry
 * of the same POST maps to the same id — idempotent under the core-records
 * (tenant_id, record_id) primary key — while distinct events keep distinct ids
 * (never collapsing an actor's whole liveness history onto one row).
 */
export function normalizeLivenessRecord(body: Record<string, unknown>): ConsoleLivenessRecord {
  ...
  const id = typeof body.id === "string" && body.id ? body.id : `liveness:${actor}:${subjectId}:${type}:${at}`;
  return {
    ...body,
