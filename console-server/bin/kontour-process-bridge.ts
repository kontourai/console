#!/usr/bin/env node
// Bridges flow-agents' local workflow-process projection envelopes
// (.kontourai/console/projections/<producer>/*.json, written by the
// `flow-agents-console-process-projection` CLI -- flow-agents#778) into a
// Console hub as `kontour.console.event` records. Read-only over the producer's
// envelope files; idempotent via content-addressed event ids + hub-side
// deduplication (console#239). --watch polls for envelope changes so the
// operating plane follows a flow-agents session's blocked/needs_input/
...
} = require("../src/console-foundation/workflow-process-bridge");
...
function printUsage(): void {
  process.stdout.write(
    `Usage: kontour-process-bridge [--projection-root ${DEFAULT_WORKFLOW_PROCESS_PROJECTION_ROOT}] [--hub http://127.0.0.1:3737]\n` +
...
function parseOptions(argv: string[], env: NodeJS.ProcessEnv = process.env): BridgeOptions {
  const options: BridgeOptions = {
    projectionRoot: DEFAULT_WORKFLOW_PROCESS_PROJECTION_ROOT,
...
function bridgeSink(options: BridgeOptions): Sink {
  return buildWorkflowProcessBridgeSink({
...
async function bridgeOnce(options: BridgeOptions, sink: Sink): Promise<void> {
  ...
  if (envelopePaths.length === 0) {
    process.stdout.write(`no workflow-process projections under ${projectionRoot}\n`);
    return;
  }
  for (const envelopePath of envelopePaths) {
    try {
      // console#239 review finding 4: thread the canonical `allowedRoot`
      // discovery already computed through to the read, so containment is
      // re-checked via realpath immediately before opening -- discovery alone
      // does not protect against a TOCTOU swap of an ancestor directory.
      const delivery = await bridgeWorkflowProcessProjection(envelopePath, sink, {
